 ...
 */
namespace MonorepoBuilder202212\Symfony\Component\VarExporter;

use MonorepoBuilder202212\Symfony\Component\VarExporter\Exception\LogicException;
use MonorepoBuilder202212\Symfony\Component\VarExporter\Internal\Hydrator;
use MonorepoBuilder202212\Symfony\Component\VarExporter\Internal\LazyObjectRegistry;
/**
...
EOPHP;
    ...
    public static function exportSignature(\ReflectionFunctionAbstract $function, bool $withParameterTypes = \true) : string
    {
        ...
        foreach ($function->getParameters() as $param) {
            $parameters[] = ((\method_exists($param, 'getAttributes') ? $param->getAttributes(\MonorepoBuilder202212\SensitiveParameter::class) : []) ? '#[\\SensitiveParameter] ' : '') . ($withParameterTypes && $param->hasType() ? self::exportType($param) . ' ' : '') . ($param->isPassedByReference() ? '&' : '') . ($param->isVariadic() ? '...' : '') . '$' . $param->name . ($param->isOptional() && !$param->isVariadic() ? ' = ' . self::exportDefault($param) : '');
        }
