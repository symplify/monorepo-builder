 ...
 */
class DecoratorServicePass extends \MonorepoBuilder20210706\Symfony\Component\DependencyInjection\Compiler\AbstractRecursivePass
{
    ...
    public function process(\MonorepoBuilder20210706\Symfony\Component\DependencyInjection\ContainerBuilder $container)
    {
        ...
        foreach ($definitions as [$id, $definition]) {
            ...
            $invalidBehavior = $decoratedService[3] ?? \MonorepoBuilder20210706\Symfony\Component\DependencyInjection\ContainerInterface::EXCEPTION_ON_INVALID_REFERENCE;
            ...
            if ($container->hasAlias($inner)) {
                ...
                $container->setAlias($renamedId, new \MonorepoBuilder20210706\Symfony\Component\DependencyInjection\Alias((string) $alias, \false));
                ...
                continue;
            } elseif (\MonorepoBuilder20210706\Symfony\Component\DependencyInjection\ContainerInterface::NULL_ON_INVALID_REFERENCE === $invalidBehavior) {
                $public = $definition->isPublic();
                $private = $definition->isPrivate();
                $decoratedDefinition = null;
            } else {
                throw new \MonorepoBuilder20210706\Symfony\Component\DependencyInjection\Exception\ServiceNotFoundException($inner, $id);
            }
            if ($decoratedDefinition && $decoratedDefinition->isSynthetic()) {
                throw new \MonorepoBuilder20210706\Symfony\Component\DependencyInjection\Exception\InvalidArgumentException(\sprintf('A synthetic service cannot be decorated: service "%s" cannot decorate "%s".', $id, $inner));
            }
    ...
    {
        if ($value instanceof \MonorepoBuilder20210706\Symfony\Component\DependencyInjection\Reference && $this->innerId === (string) $value) {
            return new \MonorepoBuilder20210706\Symfony\Component\DependencyInjection\Reference($this->currentId, $value->getInvalidBehavior());
        }
