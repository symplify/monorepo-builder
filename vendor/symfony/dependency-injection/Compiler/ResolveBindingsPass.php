 ...
 */
class ResolveBindingsPass extends \MonorepoBuilder20210706\Symfony\Component\DependencyInjection\Compiler\AbstractRecursivePass
{
    ...
    public function process(\MonorepoBuilder20210706\Symfony\Component\DependencyInjection\ContainerBuilder $container)
    {
        ...
        try {
            ...
            foreach ($this->unusedBindings as [$key, $serviceId, $bindingType, $file]) {
                ...
                if (\MonorepoBuilder20210706\Symfony\Component\DependencyInjection\Argument\BoundArgument::DEFAULTS_BINDING === $bindingType) {
                    $message .= 'under "_defaults"';
                } elseif (\MonorepoBuilder20210706\Symfony\Component\DependencyInjection\Argument\BoundArgument::INSTANCEOF_BINDING === $bindingType) {
                ...
                throw new \MonorepoBuilder20210706\Symfony\Component\DependencyInjection\Exception\InvalidArgumentException($message);
            }
        } finally {
            $this->usedBindings = [];
            $this->unusedBindings = [];
    ...
    {
        if ($value instanceof \MonorepoBuilder20210706\Symfony\Component\DependencyInjection\TypedReference && $value->getType() === (string) $value) {
            // Already checked
        ...
        if (!$value instanceof \MonorepoBuilder20210706\Symfony\Component\DependencyInjection\Definition || !($bindings = $value->getBindings())) {
            return parent::processValue($value, $isRoot);
        }
        $bindingNames = [];
        foreach ($bindings as $key => $binding) {
            ...
            if (null !== $bindingValue && !$bindingValue instanceof \MonorepoBuilder20210706\Symfony\Component\DependencyInjection\Reference && !$bindingValue instanceof \MonorepoBuilder20210706\Symfony\Component\DependencyInjection\Definition && !$bindingValue instanceof \MonorepoBuilder20210706\Symfony\Component\DependencyInjection\Argument\TaggedIteratorArgument && !$bindingValue instanceof \MonorepoBuilder20210706\Symfony\Component\DependencyInjection\Argument\ServiceLocatorArgument) {
                throw new \MonorepoBuilder20210706\Symfony\Component\DependencyInjection\Exception\InvalidArgumentException(\sprintf('Invalid value for binding key "%s" for service "%s": expected "%s", "%s", "%s", "%s" or null, "%s" given.', $key, $this->currentId, \MonorepoBuilder20210706\Symfony\Component\DependencyInjection\Reference::class, \MonorepoBuilder20210706\Symfony\Component\DependencyInjection\Definition::class, \MonorepoBuilder20210706\Symfony\Component\DependencyInjection\Argument\TaggedIteratorArgument::class, \MonorepoBuilder20210706\Symfony\Component\DependencyInjection\Argument\ServiceLocatorArgument::class, \get_debug_type($bindingValue)));
            }
        }
        if ($value->isAbstract()) {
            return parent::processValue($value, $isRoot);
        ...
        foreach ($calls as $i => $call) {
            ...
            foreach ($reflectionMethod->getParameters() as $key => $parameter) {
                ...
                $typeHint = \MonorepoBuilder20210706\Symfony\Component\DependencyInjection\LazyProxy\ProxyHelper::getTypeHint($reflectionMethod, $parameter);
                $name = \MonorepoBuilder20210706\Symfony\Component\DependencyInjection\Attribute\Target::parseName($parameter);
     ...
     * @return mixed
     */
    private function getBindingValue(\MonorepoBuilder20210706\Symfony\Component\DependencyInjection\Argument\BoundArgument $binding)
    {
