...
use MonorepoBuilder20210710\Symfony\Component\DependencyInjection\Exception\InvalidParameterTypeException;
...
use MonorepoBuilder20210710\Symfony\Component\DependencyInjection\ServiceLocator;
 ...
 */
final class CheckTypeDeclarationsPass extends \MonorepoBuilder20210710\Symfony\Component\DependencyInjection\Compiler\AbstractRecursivePass
{
    ...
    protected function processValue($value, $isRoot = \false)
    {
        ...
        if (!$value instanceof \MonorepoBuilder20210710\Symfony\Component\DependencyInjection\Definition || $value->hasErrors() || $value->isDeprecated()) {
        ...
        if (\MonorepoBuilder20210710\Symfony\Component\DependencyInjection\ServiceLocator::class === $value->getClass()) {
            return parent::processValue($value, $isRoot);
        }
     ...
     */
    private function checkTypeDeclarations(\MonorepoBuilder20210710\Symfony\Component\DependencyInjection\Definition $checkedDefinition, \ReflectionFunctionAbstract $reflectionFunction, array $values) : void
    {
        $numberOfRequiredParameters = $reflectionFunction->getNumberOfRequiredParameters();
        if (\count($values) < $numberOfRequiredParameters) {
            throw new \MonorepoBuilder20210710\Symfony\Component\DependencyInjection\Exception\InvalidArgumentException(\sprintf('Invalid definition for service "%s": "%s::%s()" requires %d arguments, %d passed.', $this->currentId, $reflectionFunction->class, $reflectionFunction->name, $numberOfRequiredParameters, \count($values)));
        }
        $reflectionParameters = $reflectionFunction->getParameters();
        $checksCount = \min($reflectionFunction->getNumberOfParameters(), \count($values));
        $envPlaceholderUniquePrefix = $this->container->getParameterBag() instanceof \MonorepoBuilder20210710\Symfony\Component\DependencyInjection\ParameterBag\EnvPlaceholderParameterBag ? $this->container->getParameterBag()->getEnvPlaceholderUniquePrefix() : null;
     ...
     */
    private function checkType(\MonorepoBuilder20210710\Symfony\Component\DependencyInjection\Definition $checkedDefinition, $value, \ReflectionParameter $parameter, ?string $envPlaceholderUniquePrefix, \ReflectionType $reflectionType = null) : void
    {
        ...
        if ($reflectionType instanceof \ReflectionUnionType) {
            foreach ($reflectionType->getTypes() as $t) {
                try {
                    ...
                    return;
                } catch (\MonorepoBuilder20210710\Symfony\Component\DependencyInjection\Exception\InvalidParameterTypeException $e) {
                }
            }
            throw new \MonorepoBuilder20210710\Symfony\Component\DependencyInjection\Exception\InvalidParameterTypeException($this->currentId, $e->getCode(), $parameter);
        }
        if (!$reflectionType instanceof \ReflectionNamedType) {
            return;
        }
        $type = $reflectionType->getName();
        if ($value instanceof \MonorepoBuilder20210710\Symfony\Component\DependencyInjection\Reference) {
            if (!$this->container->has($value = (string) $value)) {
                return;
            }
            if ('service_container' === $value && \is_a($type, \MonorepoBuilder20210710\Symfony\Component\DependencyInjection\Container::class, \true)) {
                return;
            }
        ...
        if (null === $class) {
            if ($value instanceof \MonorepoBuilder20210710\Symfony\Component\DependencyInjection\Argument\IteratorArgument) {
                $class = \MonorepoBuilder20210710\Symfony\Component\DependencyInjection\Argument\RewindableGenerator::class;
                ...
                $class = \MonorepoBuilder20210710\Symfony\Component\DependencyInjection\ServiceLocator::class;
        ...
        if ('callable' === $type && \is_array($value) && isset($value[0]) && ($value[0] instanceof \MonorepoBuilder20210710\Symfony\Component\DependencyInjection\Reference || $value[0] instanceof \MonorepoBuilder20210710\Symfony\Component\DependencyInjection\Definition || \is_string($value[0]))) {
            return;
        }
        ...
        throw new \MonorepoBuilder20210710\Symfony\Component\DependencyInjection\Exception\InvalidParameterTypeException($this->currentId, \is_object($value) ? $class : \get_debug_type($value), $parameter);
    }
    private function getExpressionLanguage() : \MonorepoBuilder20210710\Symfony\Component\DependencyInjection\ExpressionLanguage
    {
        if (null === $this->expressionLanguage) {
            $this->expressionLanguage = new \MonorepoBuilder20210710\Symfony\Component\DependencyInjection\ExpressionLanguage(null, $this->container->getExpressionLanguageProviders());
        }
