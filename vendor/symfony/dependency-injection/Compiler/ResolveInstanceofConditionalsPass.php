 ...
 */
class ResolveInstanceofConditionalsPass implements \MonorepoBuilder20220512\Symfony\Component\DependencyInjection\Compiler\CompilerPassInterface
{
    /**
     * {@inheritdoc}
     */
    public function process(\MonorepoBuilder20220512\Symfony\Component\DependencyInjection\ContainerBuilder $container)
    {
        foreach ($container->getAutoconfiguredInstanceof() as $interface => $definition) {
            if ($definition->getArguments()) {
                throw new \MonorepoBuilder20220512\Symfony\Component\DependencyInjection\Exception\InvalidArgumentException(\sprintf('Autoconfigured instanceof for type "%s" defines arguments but these are not supported and should be removed.', $interface));
            }
        }
        $tagsToKeep = [];
    ...
    private function processDefinition(\MonorepoBuilder20220512\Symfony\Component\DependencyInjection\ContainerBuilder $container, string $id, \MonorepoBuilder20220512\Symfony\Component\DependencyInjection\Definition $definition, array $tagsToKeep) : \MonorepoBuilder20220512\Symfony\Component\DependencyInjection\Definition
    {
        ...
        $parent = $definition instanceof \MonorepoBuilder20220512\Symfony\Component\DependencyInjection\ChildDefinition ? $definition->getParent() : null;
        ...
        if ($parent) {
            ...
            if (\MonorepoBuilder20220512\Symfony\Component\DependencyInjection\Definition::class === \get_class($abstract)) {
    ...
    private function mergeConditionals(array $autoconfiguredInstanceof, array $instanceofConditionals, \MonorepoBuilder20220512\Symfony\Component\DependencyInjection\ContainerBuilder $container) : array
    {
        ...
        foreach ($instanceofConditionals as $interface => $instanceofDef) {
            ...
            if (!$container->getReflectionClass($interface)) {
                throw new \MonorepoBuilder20220512\Symfony\Component\DependencyInjection\Exception\RuntimeException(\sprintf('"%s" is set as an "instanceof" conditional, but it does not exist.', $interface));
            }
