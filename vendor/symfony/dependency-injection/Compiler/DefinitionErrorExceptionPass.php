 ...
 */
class DefinitionErrorExceptionPass extends \MonorepoBuilder20210708\Symfony\Component\DependencyInjection\Compiler\AbstractRecursivePass
{
    /**
     * {@inheritdoc}
     */
    ...
    {
        if (!$value instanceof \MonorepoBuilder20210708\Symfony\Component\DependencyInjection\Definition || !$value->hasErrors()) {
        ...
        if ($isRoot && !$value->isPublic()) {
            ...
            foreach ($graph->getNode($this->currentId)->getInEdges() as $edge) {
                if (!$edge->getValue() instanceof \MonorepoBuilder20210708\Symfony\Component\DependencyInjection\Reference || \MonorepoBuilder20210708\Symfony\Component\DependencyInjection\ContainerInterface::RUNTIME_EXCEPTION_ON_INVALID_REFERENCE !== $edge->getValue()->getInvalidBehavior()) {
                    $runtimeException = \false;
                    break;
                }
