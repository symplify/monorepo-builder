 ...
 */
class EnvPlaceholderParameterBag extends \MonorepoBuilder20220418\Symfony\Component\DependencyInjection\ParameterBag\ParameterBag
{
    /**
     * @var string
     */
    ...
    public function get(string $name)
    {
        if (\strncmp($name, 'env(', \strlen('env(')) === 0 && \substr_compare($name, ')', -\strlen(')')) === 0 && 'env()' !== $name) {
            ...
            if (!\preg_match('/^(?:[-.\\w]*+:)*+\\w++$/', $env)) {
                throw new \MonorepoBuilder20220418\Symfony\Component\DependencyInjection\Exception\InvalidArgumentException(\sprintf('Invalid %s name: only "word" characters are allowed.', $name));
            }
            if ($this->has($name) && null !== ($defaultValue = parent::get($name)) && !\is_string($defaultValue)) {
                throw new \MonorepoBuilder20220418\Symfony\Component\DependencyInjection\Exception\RuntimeException(\sprintf('The default value of an env() parameter must be a string or null, but "%s" given to "%s".', \get_debug_type($defaultValue), $name));
            }
    ...
    public function resolve()
    {
        ...
        foreach ($this->envPlaceholders as $env => $placeholders) {
            if ($this->has($name = "env({$env})") && null !== ($default = $this->parameters[$name]) && !\is_string($default)) {
                throw new \MonorepoBuilder20220418\Symfony\Component\DependencyInjection\Exception\RuntimeException(\sprintf('The default value of env parameter "%s" must be a string or null, "%s" given.', $env, \get_debug_type($default)));
            }
        }
    }
}
