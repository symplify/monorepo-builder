...
use MonorepoBuilder20210811\Symfony\Component\DependencyInjection\Exception\OutOfBoundsException;
/**
...
class Definition
{
    ...
    public function setFactory($factory)
    {
        ...
        } elseif ($factory instanceof \MonorepoBuilder20210811\Symfony\Component\DependencyInjection\Reference) {
     ...
     */
    public function setDecoratedService($id, $renamedId = null, $priority = 0, $invalidBehavior = \MonorepoBuilder20210811\Symfony\Component\DependencyInjection\ContainerInterface::EXCEPTION_ON_INVALID_REFERENCE)
    {
        if ($renamedId && $id === $renamedId) {
            throw new \MonorepoBuilder20210811\Symfony\Component\DependencyInjection\Exception\InvalidArgumentException(\sprintf('The decorated service inner name for "%s" must be different than the service name itself.', $id));
        }
    ...
    public function replaceArgument($index, $argument)
    {
        if (0 === \count($this->arguments)) {
            throw new \MonorepoBuilder20210811\Symfony\Component\DependencyInjection\Exception\OutOfBoundsException('Cannot replace arguments if none have been configured yet.');
        }
        if (\is_int($index) && ($index < 0 || $index > \count($this->arguments) - 1)) {
            throw new \MonorepoBuilder20210811\Symfony\Component\DependencyInjection\Exception\OutOfBoundsException(\sprintf('The index "%d" is not in the range [0, %d].', $index, \count($this->arguments) - 1));
        }
        if (!\array_key_exists($index, $this->arguments)) {
            throw new \MonorepoBuilder20210811\Symfony\Component\DependencyInjection\Exception\OutOfBoundsException(\sprintf('The argument "%s" doesn\'t exist.', $index));
        }
    ...
    {
        if (empty($method)) {
            throw new \MonorepoBuilder20210811\Symfony\Component\DependencyInjection\Exception\InvalidArgumentException('Method name cannot be empty.');
        }
    ...
    public function setDeprecated()
    {
        ...
        if ('' !== $message) {
            if (\preg_match('#[\\r\\n]|\\*/#', $message)) {
                throw new \MonorepoBuilder20210811\Symfony\Component\DependencyInjection\Exception\InvalidArgumentException('Invalid characters found in deprecation template.');
            }
            if (\strpos($message, '%service_id%') === \false) {
                throw new \MonorepoBuilder20210811\Symfony\Component\DependencyInjection\Exception\InvalidArgumentException('The deprecation template must contain the "%service_id%" placeholder.');
            }
        }
    ...
    public function setConfigurator($configurator)
    {
        ...
        } elseif ($configurator instanceof \MonorepoBuilder20210811\Symfony\Component\DependencyInjection\Reference) {
    ...
    public function setBindings($bindings)
    {
        foreach ($bindings as $key => $binding) {
            ...
            if (!$binding instanceof \MonorepoBuilder20210811\Symfony\Component\DependencyInjection\Argument\BoundArgument) {
                $bindings[$key] = new \MonorepoBuilder20210811\Symfony\Component\DependencyInjection\Argument\BoundArgument($binding);
            }
        }
