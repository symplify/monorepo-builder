...
use MonorepoBuilder20210707\Symfony\Component\HttpFoundation\Exception\JsonException;
use MonorepoBuilder20210707\Symfony\Component\HttpFoundation\Exception\SessionNotFoundException;
use MonorepoBuilder20210707\Symfony\Component\HttpFoundation\Exception\SuspiciousOperationException;
use MonorepoBuilder20210707\Symfony\Component\HttpFoundation\Session\SessionInterface;
// Help opcache.preload discover always-needed symbols
\class_exists(\MonorepoBuilder20210707\Symfony\Component\HttpFoundation\AcceptHeader::class);
\class_exists(\MonorepoBuilder20210707\Symfony\Component\HttpFoundation\FileBag::class);
\class_exists(\MonorepoBuilder20210707\Symfony\Component\HttpFoundation\HeaderBag::class);
\class_exists(\MonorepoBuilder20210707\Symfony\Component\HttpFoundation\HeaderUtils::class);
\class_exists(\MonorepoBuilder20210707\Symfony\Component\HttpFoundation\InputBag::class);
\class_exists(\MonorepoBuilder20210707\Symfony\Component\HttpFoundation\ParameterBag::class);
\class_exists(\MonorepoBuilder20210707\Symfony\Component\HttpFoundation\ServerBag::class);
/**
...
class Request
{
    ...
    {
        $this->request = new \MonorepoBuilder20210707\Symfony\Component\HttpFoundation\InputBag($request);
        $this->query = new \MonorepoBuilder20210707\Symfony\Component\HttpFoundation\InputBag($query);
        $this->attributes = new \MonorepoBuilder20210707\Symfony\Component\HttpFoundation\ParameterBag($attributes);
        $this->cookies = new \MonorepoBuilder20210707\Symfony\Component\HttpFoundation\InputBag($cookies);
        $this->files = new \MonorepoBuilder20210707\Symfony\Component\HttpFoundation\FileBag($files);
        $this->server = new \MonorepoBuilder20210707\Symfony\Component\HttpFoundation\ServerBag($server);
        $this->headers = new \MonorepoBuilder20210707\Symfony\Component\HttpFoundation\HeaderBag($this->server->getHeaders());
    ...
    public static function createFromGlobals()
    {
        ...
        if (0 === \strpos($request->headers->get('CONTENT_TYPE', ''), 'application/x-www-form-urlencoded') && \in_array(\strtoupper($request->server->get('REQUEST_METHOD', 'GET')), ['PUT', 'DELETE', 'PATCH'])) {
            ...
            $request->request = new \MonorepoBuilder20210707\Symfony\Component\HttpFoundation\InputBag($data);
        }
        return $request;
    }
    /**
    ...
    public function duplicate(array $query = null, array $request = null, array $attributes = null, array $cookies = null, array $files = null, array $server = null)
    {
        $dup = clone $this;
        if (null !== $query) {
            $dup->query = new \MonorepoBuilder20210707\Symfony\Component\HttpFoundation\InputBag($query);
        }
        if (null !== $request) {
            $dup->request = new \MonorepoBuilder20210707\Symfony\Component\HttpFoundation\InputBag($request);
        }
        if (null !== $attributes) {
            $dup->attributes = new \MonorepoBuilder20210707\Symfony\Component\HttpFoundation\ParameterBag($attributes);
        }
        if (null !== $cookies) {
            $dup->cookies = new \MonorepoBuilder20210707\Symfony\Component\HttpFoundation\InputBag($cookies);
        }
        if (null !== $files) {
            $dup->files = new \MonorepoBuilder20210707\Symfony\Component\HttpFoundation\FileBag($files);
        }
        if (null !== $server) {
            $dup->server = new \MonorepoBuilder20210707\Symfony\Component\HttpFoundation\ServerBag($server);
            $dup->headers = new \MonorepoBuilder20210707\Symfony\Component\HttpFoundation\HeaderBag($dup->server->getHeaders());
        }
        $dup->languages = null;
        $dup->charsets = null;
        $dup->encodings = null;
    ...
    public static function normalizeQueryString(?string $qs)
    {
        ...
        $qs = \MonorepoBuilder20210707\Symfony\Component\HttpFoundation\HeaderUtils::parseQuery($qs);
        \ksort($qs);
    ...
    public function getSession()
    {
        ...
        if (!$session instanceof \MonorepoBuilder20210707\Symfony\Component\HttpFoundation\Session\SessionInterface && null !== $session) {
            $this->setSession($session = $session());
        }
        if (null === $session) {
            throw new \MonorepoBuilder20210707\Symfony\Component\HttpFoundation\Exception\SessionNotFoundException('Session has not been set.');
        }
        return $session;
    }
    /**
    ...
    public function setSession(\MonorepoBuilder20210707\Symfony\Component\HttpFoundation\Session\SessionInterface $session)
    {
    ...
    public function getHost()
    {
        ...
        if ($host && '' !== \preg_replace('/(?:^\\[)?[a-zA-Z0-9-:\\]_]+\\.?/', '', $host)) {
            ...
            throw new \MonorepoBuilder20210707\Symfony\Component\HttpFoundation\Exception\SuspiciousOperationException(\sprintf('Invalid Host "%s".', $host));
        }
        if (\count(self::$trustedHostPatterns) > 0) {
            ...
            $this->isHostValid = \false;
            throw new \MonorepoBuilder20210707\Symfony\Component\HttpFoundation\Exception\SuspiciousOperationException(\sprintf('Untrusted Host "%s".', $host));
        }
        return $host;
    }
    /**
    ...
    public function getMethod()
    {
        ...
        if (!\preg_match('/^[A-Z]++$/D', $method)) {
            throw new \MonorepoBuilder20210707\Symfony\Component\HttpFoundation\Exception\SuspiciousOperationException(\sprintf('Invalid method override "%s".', $method));
        }
        return $this->method = $method;
    }
    /**
    ...
    public function toArray()
    {
        if ('' === ($content = $this->getContent())) {
            throw new \MonorepoBuilder20210707\Symfony\Component\HttpFoundation\Exception\JsonException('Request body is empty.');
        }
        try {
            $content = \json_decode($content, \true, 512, \JSON_BIGINT_AS_STRING | (\PHP_VERSION_ID >= 70300 ? \JSON_THROW_ON_ERROR : 0));
        } catch (\JsonException $e) {
            throw new \MonorepoBuilder20210707\Symfony\Component\HttpFoundation\Exception\JsonException('Could not decode request body.', $e->getCode(), $e);
        }
        if (\PHP_VERSION_ID < 70300 && \JSON_ERROR_NONE !== \json_last_error()) {
            throw new \MonorepoBuilder20210707\Symfony\Component\HttpFoundation\Exception\JsonException('Could not decode request body: ' . \json_last_error_msg(), \json_last_error());
        }
        if (!\is_array($content)) {
            throw new \MonorepoBuilder20210707\Symfony\Component\HttpFoundation\Exception\JsonException(\sprintf('JSON content was expected to decode to an array, "%s" returned.', \get_debug_type($content)));
        }
        return $content;
    }
    /**
    ...
    public function getLanguages()
    {
        ...
        $languages = \MonorepoBuilder20210707\Symfony\Component\HttpFoundation\AcceptHeader::fromString($this->headers->get('Accept-Language'))->all();
        $this->languages = [];
    ...
    public function getCharsets()
    {
        ...
        return $this->charsets = \array_keys(\MonorepoBuilder20210707\Symfony\Component\HttpFoundation\AcceptHeader::fromString($this->headers->get('Accept-Charset'))->all());
    }
    /**
    ...
    public function getEncodings()
    {
        ...
        return $this->encodings = \array_keys(\MonorepoBuilder20210707\Symfony\Component\HttpFoundation\AcceptHeader::fromString($this->headers->get('Accept-Encoding'))->all());
    }
    /**
    ...
    public function getAcceptableContentTypes()
    {
        ...
        return $this->acceptableContentTypes = \array_keys(\MonorepoBuilder20210707\Symfony\Component\HttpFoundation\AcceptHeader::fromString($this->headers->get('Accept'))->all());
    }
    /**
    ...
    public function preferSafeContent() : bool
    {
        ...
        $this->isSafeContentPreferred = \MonorepoBuilder20210707\Symfony\Component\HttpFoundation\AcceptHeader::fromString($this->headers->get('Prefer'))->has('safe');
        return $this->isSafeContentPreferred;
    }
    /*
    ...
    {
        return self::$trustedProxies && \MonorepoBuilder20210707\Symfony\Component\HttpFoundation\IpUtils::checkIp($this->server->get('REMOTE_ADDR', ''), self::$trustedProxies);
    }
    private function getTrustedValues(int $type, string $ip = null) : array
    {
        ...
        if (self::$trustedHeaderSet & self::HEADER_FORWARDED && isset(self::FORWARDED_PARAMS[$type]) && $this->headers->has(self::TRUSTED_HEADERS[self::HEADER_FORWARDED])) {
            ...
            $parts = \MonorepoBuilder20210707\Symfony\Component\HttpFoundation\HeaderUtils::split($forwarded, ',;=');
            $forwardedValues = [];
            $param = self::FORWARDED_PARAMS[$type];
            foreach ($parts as $subParts) {
                if (null === ($v = \MonorepoBuilder20210707\Symfony\Component\HttpFoundation\HeaderUtils::combine($subParts)[$param] ?? null)) {
                    continue;
                }
    ...
    private function normalizeAndFilterClientIps(array $clientIps, string $ip) : array
    {
        ...
        foreach ($clientIps as $key => $clientIp) {
            ...
            if (\MonorepoBuilder20210707\Symfony\Component\HttpFoundation\IpUtils::checkIp($clientIp, self::$trustedProxies)) {
                unset($clientIps[$key]);
