...
use MonorepoBuilder20211007\Symfony\Component\HttpFoundation\File\File;
/**
 ...
 */
class BinaryFileResponse extends \MonorepoBuilder20211007\Symfony\Component\HttpFoundation\Response
{
    ...
    {
        if (!$file instanceof \MonorepoBuilder20211007\Symfony\Component\HttpFoundation\File\File) {
            if ($file instanceof \SplFileInfo) {
                $file = new \MonorepoBuilder20211007\Symfony\Component\HttpFoundation\File\File($file->getPathname());
            } else {
                $file = new \MonorepoBuilder20211007\Symfony\Component\HttpFoundation\File\File((string) $file);
            }
        }
        if (!$file->isReadable()) {
            throw new \MonorepoBuilder20211007\Symfony\Component\HttpFoundation\File\Exception\FileException('File must be readable.');
        }
        $this->file = $file;
        if ($autoEtag) {
            $this->setAutoEtag();
    ...
    public function prepare($request)
    {
        ...
        if (self::$trustXSendfileTypeHeader && $request->headers->has('X-Sendfile-Type')) {
            ...
            if ('x-accel-redirect' === \strtolower($type)) {
                ...
                $parts = \MonorepoBuilder20211007\Symfony\Component\HttpFoundation\HeaderUtils::split($request->headers->get('X-Accel-Mapping', ''), ',=');
