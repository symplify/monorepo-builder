...
use MonorepoBuilder20210706\Symfony\Component\HttpKernel\Exception\UnexpectedSessionUsageException;
 ...
 * @internal
 */
abstract class AbstractSessionListener implements \MonorepoBuilder20210706\Symfony\Component\EventDispatcher\EventSubscriberInterface
{
    public const NO_AUTO_CACHE_CONTROL_HEADER = 'Symfony-Session-NoAutoCacheControl';
    protected $container;
    private $sessionUsageStack = [];
    private $debug;
    public function __construct(\MonorepoBuilder20210706\Psr\Container\ContainerInterface $container = null, bool $debug = \false)
    {
    ...
    public function onKernelRequest(\MonorepoBuilder20210706\Symfony\Component\HttpKernel\Event\RequestEvent $event)
    {
        ...
        $this->sessionUsageStack[] = $session instanceof \MonorepoBuilder20210706\Symfony\Component\HttpFoundation\Session\Session ? $session->getUsageIndex() : 0;
    }
    public function onKernelResponse(\MonorepoBuilder20210706\Symfony\Component\HttpKernel\Event\ResponseEvent $event)
    {
        ...
        if ($session instanceof \MonorepoBuilder20210706\Symfony\Component\HttpFoundation\Session\Session ? $session->getUsageIndex() === \end($this->sessionUsageStack) : !$session->isStarted()) {
            return;
        }
        if ($autoCacheControl) {
        ...
        if ($this->debug) {
            throw new \MonorepoBuilder20210706\Symfony\Component\HttpKernel\Exception\UnexpectedSessionUsageException('Session was used while the request was declared stateless.');
        }
        if ($this->container->has('logger')) {
            $this->container->get('logger')->warning('Session was used while the request was declared stateless.');
        }
    }
    public function onFinishRequest(\MonorepoBuilder20210706\Symfony\Component\HttpKernel\Event\FinishRequestEvent $event)
    {
        if ($event->isMainRequest()) {
    ...
    {
        return [
            \MonorepoBuilder20210706\Symfony\Component\HttpKernel\KernelEvents::REQUEST => ['onKernelRequest', 128],
            // low priority to come after regular response listeners, but higher than StreamedResponseListener
            \MonorepoBuilder20210706\Symfony\Component\HttpKernel\KernelEvents::RESPONSE => ['onKernelResponse', -1000],
            \MonorepoBuilder20210706\Symfony\Component\HttpKernel\KernelEvents::FINISH_REQUEST => ['onFinishRequest'],
        ];
    }
    /**
