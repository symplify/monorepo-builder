...
use MonorepoBuilder20210707\Symfony\Component\VarDumper\Dumper\DataDumperInterface;
use MonorepoBuilder20210707\Symfony\Component\VarDumper\Dumper\HtmlDumper;
use MonorepoBuilder20210707\Symfony\Component\VarDumper\Server\Connection;
/**
 * @author Nicolas Grekas <p@tchwork.com>
 *
 * @final
 */
class DumpDataCollector extends \MonorepoBuilder20210707\Symfony\Component\HttpKernel\DataCollector\DataCollector implements \MonorepoBuilder20210707\Symfony\Component\VarDumper\Dumper\DataDumperInterface
{
    private $stopwatch;
     ...
     */
    public function __construct(\MonorepoBuilder20210707\Symfony\Component\Stopwatch\Stopwatch $stopwatch = null, $fileLinkFormat = null, string $charset = null, \MonorepoBuilder20210707\Symfony\Component\HttpFoundation\RequestStack $requestStack = null, $dumper = null)
    {
        ...
        $this->sourceContextProvider = $dumper instanceof \MonorepoBuilder20210707\Symfony\Component\VarDumper\Server\Connection && isset($dumper->getContextProviders()['source']) ? $dumper->getContextProviders()['source'] : new \MonorepoBuilder20210707\Symfony\Component\VarDumper\Dumper\ContextProvider\SourceContextProvider($this->charset);
    }
    ...
    public function dump(\MonorepoBuilder20210707\Symfony\Component\VarDumper\Cloner\Data $data)
    {
        ...
        if ($this->dumper instanceof \MonorepoBuilder20210707\Symfony\Component\VarDumper\Server\Connection) {
    ...
    public function collect(\MonorepoBuilder20210707\Symfony\Component\HttpFoundation\Request $request, \MonorepoBuilder20210707\Symfony\Component\HttpFoundation\Response $response, \Throwable $exception = null)
    {
        ...
        if (!$this->requestStack || !$response->headers->has('X-Debug-Token') || $response->isRedirection() || $response->headers->has('Content-Type') && \false === \strpos($response->headers->get('Content-Type'), 'html') || 'html' !== $request->getRequestFormat() || \false === \strripos($response->getContent(), '</body>')) {
            if ($response->headers->has('Content-Type') && \false !== \strpos($response->headers->get('Content-Type'), 'html')) {
                $dumper = new \MonorepoBuilder20210707\Symfony\Component\VarDumper\Dumper\HtmlDumper('php://output', $this->charset);
                $dumper->setDisplayOptions(['fileLinkFormat' => $this->fileLinkFormat]);
            } else {
                $dumper = new \MonorepoBuilder20210707\Symfony\Component\VarDumper\Dumper\CliDumper('php://output', $this->charset);
    ...
    public function __wakeup()
    {
        ...
        self::__construct($this->stopwatch, \is_string($fileLinkFormat) || $fileLinkFormat instanceof \MonorepoBuilder20210707\Symfony\Component\HttpKernel\Debug\FileLinkFormatter ? $fileLinkFormat : null, \is_string($charset) ? $charset : null);
    }
    ...
    public function getDumps($format, $maxDepthLimit = -1, $maxItemsPerDepth = -1) : array
    {
        ...
        if ('html' === $format) {
            $dumper = new \MonorepoBuilder20210707\Symfony\Component\VarDumper\Dumper\HtmlDumper($data, $this->charset);
            $dumper->setDisplayOptions(['fileLinkFormat' => $this->fileLinkFormat]);
        } else {
    ...
    private function doDump(\MonorepoBuilder20210707\Symfony\Component\VarDumper\Dumper\DataDumperInterface $dumper, \MonorepoBuilder20210707\Symfony\Component\VarDumper\Cloner\Data $data, string $name, string $file, int $line)
    {
        if ($dumper instanceof \MonorepoBuilder20210707\Symfony\Component\VarDumper\Dumper\CliDumper) {
            $contextDumper = function ($name, $file, $line, $fmt) {
                if ($this instanceof \MonorepoBuilder20210707\Symfony\Component\VarDumper\Dumper\HtmlDumper) {
                    if ($file) {
                        $s = $this->style('meta', '%s');
