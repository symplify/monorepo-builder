 ...
 * @final
 */
class LoggerDataCollector extends \MonorepoBuilder20210912\Symfony\Component\HttpKernel\DataCollector\DataCollector implements \MonorepoBuilder20210912\Symfony\Component\HttpKernel\DataCollector\LateDataCollectorInterface
{
    private $logger;
     ...
     * @param object $logger
     */
    public function __construct($logger = null, string $containerPathPrefix = null, \MonorepoBuilder20210912\Symfony\Component\HttpFoundation\RequestStack $requestStack = null)
    {
        if (null !== $logger && $logger instanceof \MonorepoBuilder20210912\Symfony\Component\HttpKernel\Log\DebugLoggerInterface) {
            $this->logger = $logger;
        }
    ...
    {
        if ($this->logger instanceof \MonorepoBuilder20210912\Symfony\Component\HttpKernel\Log\DebugLoggerInterface) {
            $this->logger->clear();
        }
        $this->data = [];
    }
    ...
    private function getContainerDeprecationLogs() : array
    {
        ...
        foreach (\unserialize($logContent) as $log) {
            $log['context'] = ['exception' => new \MonorepoBuilder20210912\Symfony\Component\ErrorHandler\Exception\SilencedErrorContext($log['type'], $log['file'], $log['line'], $log['trace'], $log['count'])];
    ...
    private function sanitizeLogs(array $logs)
    {
        ...
        foreach ($logs as $log) {
            ...
            if ($exception instanceof \MonorepoBuilder20210912\Symfony\Component\ErrorHandler\Exception\SilencedErrorContext) {
