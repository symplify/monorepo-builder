 ...
 */
class RegisterLocaleAwareServicesPass implements \MonorepoBuilder20210727\Symfony\Component\DependencyInjection\Compiler\CompilerPassInterface
{
    ...
    public function process($container)
    {
        ...
        foreach ($container->findTaggedServiceIds($this->localeAwareTag) as $id => $tags) {
            $services[] = new \MonorepoBuilder20210727\Symfony\Component\DependencyInjection\Reference($id);
        }
        if (!$services) {
            $container->removeDefinition($this->listenerServiceId);
            return;
        }
        $container->getDefinition($this->listenerServiceId)->setArgument(0, new \MonorepoBuilder20210727\Symfony\Component\DependencyInjection\Argument\IteratorArgument($services));
    }
}
