...
final class ProgressBar
{
     ...
     */
    public function __construct(\MonorepoBuilder20211130\Symfony\Component\Console\Output\OutputInterface $output, int $max = 0, float $minSecondsBetweenRedraws = 1 / 25)
    {
        ...
        $this->startTime = \time();
        $this->cursor = new \MonorepoBuilder20211130\Symfony\Component\Console\Cursor($output);
    }
    /**
    ...
    public function setMaxSteps(int $max)
    {
        $this->format = null;
        $this->max = \max(0, $max);
        $this->stepWidth = $this->max ? \MonorepoBuilder20211130\Symfony\Component\Console\Helper\Helper::width((string) $this->max) : 4;
    }
    /**
            ...
            if (null !== $this->previousMessage) {
                if ($this->output instanceof \MonorepoBuilder20211130\Symfony\Component\Console\Output\ConsoleSectionOutput) {
                    ...
                    foreach ($messageLines as $messageLine) {
                        $messageLineLength = \MonorepoBuilder20211130\Symfony\Component\Console\Helper\Helper::width(\MonorepoBuilder20211130\Symfony\Component\Console\Helper\Helper::removeDecoration($this->output->getFormatter(), $messageLine));
    ...
    {
        return ['bar' => function (self $bar, \MonorepoBuilder20211130\Symfony\Component\Console\Output\OutputInterface $output) {
            $completeBars = $bar->getBarOffset();
            $display = \str_repeat($bar->getBarCharacter(), $completeBars);
            if ($completeBars < $bar->getBarWidth()) {
                $emptyBars = $bar->getBarWidth() - $completeBars - \MonorepoBuilder20211130\Symfony\Component\Console\Helper\Helper::length(\MonorepoBuilder20211130\Symfony\Component\Console\Helper\Helper::removeDecoration($output->getFormatter(), $bar->getProgressCharacter()));
                $display .= $bar->getProgressCharacter() . \str_repeat($bar->getEmptyBarCharacter(), $emptyBars);
            }
            return $display;
        }, 'elapsed' => function (self $bar) {
            return \MonorepoBuilder20211130\Symfony\Component\Console\Helper\Helper::formatTime(\time() - $bar->getStartTime());
        }, 'remaining' => function (self $bar) {
            if (!$bar->getMaxSteps()) {
                throw new \MonorepoBuilder20211130\Symfony\Component\Console\Exception\LogicException('Unable to display the remaining time if the maximum number of steps is not set.');
            }
            return \MonorepoBuilder20211130\Symfony\Component\Console\Helper\Helper::formatTime($bar->getRemaining());
        }, 'estimated' => function (self $bar) {
            if (!$bar->getMaxSteps()) {
                throw new \MonorepoBuilder20211130\Symfony\Component\Console\Exception\LogicException('Unable to display the estimated time if the maximum number of steps is not set.');
            }
            return \MonorepoBuilder20211130\Symfony\Component\Console\Helper\Helper::formatTime($bar->getEstimated());
    ...
    private function buildLine() : string
    {
        ...
        $linesLength = \array_map(function ($subLine) {
            return \MonorepoBuilder20211130\Symfony\Component\Console\Helper\Helper::width(\MonorepoBuilder20211130\Symfony\Component\Console\Helper\Helper::removeDecoration($this->output->getFormatter(), \rtrim($subLine, "\r")));
        }, \explode("\n", $line));
