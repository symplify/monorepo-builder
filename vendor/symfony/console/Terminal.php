...
use MonorepoBuilderPrefix202507\Symfony\Component\Console\Output\AnsiColorMode;
class Terminal
{
    ...
    private static function readFromProcess($command) : ?string
    {
        ...
        if (!($process = @\proc_open(\is_array($command) ? \implode(' ', $command) : $command, $descriptorspec, $pipes, null, null, ['suppress_errors' => \true]))) {
            return null;
        }
