...
use MonorepoBuilder20211020\Doctrine\Common\Persistence\Proxy as LegacyProxy;
use MonorepoBuilder20211020\Doctrine\Persistence\Proxy;
use MonorepoBuilder20211020\Mockery\MockInterface;
use MonorepoBuilder20211020\Phake\IMock;
use MonorepoBuilder20211020\PHPUnit\Framework\MockObject\Matcher\StatelessInvocation;
use MonorepoBuilder20211020\PHPUnit\Framework\MockObject\MockObject;
use MonorepoBuilder20211020\Prophecy\Prophecy\ProphecySubjectInterface;
use MonorepoBuilder20211020\ProxyManager\Proxy\ProxyInterface;
/**
...
class DebugClassLoader
{
    ...
    public static function enable() : void
    {
        ...
        \class_exists(\MonorepoBuilder20211020\Symfony\Component\ErrorHandler\ErrorHandler::class);
        \class_exists(\MonorepoBuilder20211020\Psr\Log\LogLevel::class);
    ...
    public static function checkClasses() : bool
    {
        ...
        foreach ($offsets as $getSymbols => $i) {
            ...
            for (; $i < \count($symbols); ++$i) {
                if (!\is_subclass_of($symbols[$i], \MonorepoBuilder20211020\PHPUnit\Framework\MockObject\MockObject::class) && !\is_subclass_of($symbols[$i], \MonorepoBuilder20211020\Prophecy\Prophecy\ProphecySubjectInterface::class) && !\is_subclass_of($symbols[$i], \MonorepoBuilder20211020\Doctrine\Persistence\Proxy::class) && !\is_subclass_of($symbols[$i], \MonorepoBuilder20211020\ProxyManager\Proxy\ProxyInterface::class) && !\is_subclass_of($symbols[$i], \MonorepoBuilder20211020\Doctrine\Common\Persistence\Proxy::class) && !\is_subclass_of($symbols[$i], \MonorepoBuilder20211020\Mockery\MockInterface::class) && !\is_subclass_of($symbols[$i], \MonorepoBuilder20211020\Phake\IMock::class)) {
    ...
    public function checkAnnotations($refl, $class) : array
    {
        ...
        foreach ($parentAndOwnInterfaces + \class_uses($class, \false) as $use) {
            ...
            if (isset(self::$method[$use])) {
                if ($refl->isAbstract()) {
                    ...
                    if (!\strncmp($vendor, \str_replace('_', '\\', $use), $vendorLen) && 0 === \strpos($className, 'Symfony\\') && (!\class_exists(\MonorepoBuilder20211020\Composer\InstalledVersions::class) || 'symfony/symfony' !== \MonorepoBuilder20211020\Composer\InstalledVersions::getRootPackage()['name'])) {
        ...
        foreach ($refl->getMethods() as $method) {
            ...
            if ($finalOrInternal || $method->isConstructor() || \false === \strpos($doc, '@param') || \MonorepoBuilder20211020\PHPUnit\Framework\MockObject\Matcher\StatelessInvocation::class === $class) {
                continue;
            }
