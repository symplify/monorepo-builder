...
use MonorepoBuilder20210706\Symfony\Component\VarDumper\Cloner\Cursor;
 ...
 */
class CliDumper extends \MonorepoBuilder20210706\Symfony\Component\VarDumper\Dumper\AbstractDumper
{
     ...
     * {@inheritdoc}
     */
    public function dumpScalar(\MonorepoBuilder20210706\Symfony\Component\VarDumper\Cloner\Cursor $cursor, string $type, $value)
    {
        $this->dumpKey($cursor);
        $style = 'const';
        $attr = $cursor->attr;
     ...
     * {@inheritdoc}
     */
    public function dumpString(\MonorepoBuilder20210706\Symfony\Component\VarDumper\Cloner\Cursor $cursor, string $str, bool $bin, int $cut)
    {
        $this->dumpKey($cursor);
        $attr = $cursor->attr;
        if ($bin) {
     ...
     * {@inheritdoc}
     */
    public function enterHash(\MonorepoBuilder20210706\Symfony\Component\VarDumper\Cloner\Cursor $cursor, int $type, $class, bool $hasChild)
    {
        if (null === $this->colors) {
        ...
        if (\MonorepoBuilder20210706\Symfony\Component\VarDumper\Cloner\Cursor::HASH_OBJECT === $type) {
            $prefix = $class && 'stdClass' !== $class ? $this->style('note', $class, $attr) . (empty($attr['cut_hash']) ? ' {' : '') : '{';
        } elseif (\MonorepoBuilder20210706\Symfony\Component\VarDumper\Cloner\Cursor::HASH_RESOURCE === $type) {
            $prefix = $this->style('note', $class . ' resource', $attr) . ($hasChild ? ' {' : ' ');
        } else {
            $prefix = $class && !(self::DUMP_LIGHT_ARRAY & $this->flags) ? $this->style('note', 'array:' . $class) . ' [' : '[';
        }
        if (($cursor->softRefCount || 0 < $cursor->softRefHandle) && empty($attr['cut_hash'])) {
            $prefix .= $this->style('ref', (\MonorepoBuilder20210706\Symfony\Component\VarDumper\Cloner\Cursor::HASH_RESOURCE === $type ? '@' : '#') . (0 < $cursor->softRefHandle ? $cursor->softRefHandle : $cursor->softRefTo), ['count' => $cursor->softRefCount]);
        } elseif ($cursor->hardRefTo && !$cursor->refIndex && $class) {
            $prefix .= $this->style('ref', '&' . $cursor->hardRefTo, ['count' => $cursor->hardRefCount]);
        } elseif (!$hasChild && \MonorepoBuilder20210706\Symfony\Component\VarDumper\Cloner\Cursor::HASH_RESOURCE === $type) {
     ...
     * {@inheritdoc}
     */
    public function leaveHash(\MonorepoBuilder20210706\Symfony\Component\VarDumper\Cloner\Cursor $cursor, int $type, $class, bool $hasChild, int $cut)
    {
        if (empty($cursor->attr['cut_hash'])) {
            $this->dumpEllipsis($cursor, $hasChild, $cut);
            $this->line .= \MonorepoBuilder20210706\Symfony\Component\VarDumper\Cloner\Cursor::HASH_OBJECT === $type ? '}' : (\MonorepoBuilder20210706\Symfony\Component\VarDumper\Cloner\Cursor::HASH_RESOURCE !== $type ? ']' : ($hasChild ? '}' : ''));
        }
        $this->endValue($cursor);
    }
    /**
     ...
     */
    protected function dumpEllipsis(\MonorepoBuilder20210706\Symfony\Component\VarDumper\Cloner\Cursor $cursor, $hasChild, $cut)
    {
        if ($cut) {
            $this->line .= ' …';
            if (0 < $cut) {
     ...
     */
    protected function dumpKey(\MonorepoBuilder20210706\Symfony\Component\VarDumper\Cloner\Cursor $cursor)
    {
        if (null !== ($key = $cursor->hashKey)) {
            ...
            switch ($cursor->hashType) {
                default:
                case \MonorepoBuilder20210706\Symfony\Component\VarDumper\Cloner\Cursor::HASH_INDEXED:
                    if (self::DUMP_LIGHT_ARRAY & $this->flags) {
                        break;
                    }
                    $style = 'index';
                // no break
                case \MonorepoBuilder20210706\Symfony\Component\VarDumper\Cloner\Cursor::HASH_ASSOC:
                    if (\is_int($key)) {
                    ...
                    break;
                case \MonorepoBuilder20210706\Symfony\Component\VarDumper\Cloner\Cursor::HASH_RESOURCE:
                    $key = "\0~\0" . $key;
                // no break
                case \MonorepoBuilder20210706\Symfony\Component\VarDumper\Cloner\Cursor::HASH_OBJECT:
    ...
    protected function endValue(\MonorepoBuilder20210706\Symfony\Component\VarDumper\Cloner\Cursor $cursor)
    {
        if (-1 === $cursor->hashType) {
            return;
        }
        if (\MonorepoBuilder20210706\Symfony\Component\VarDumper\Cloner\Stub::ARRAY_INDEXED === $cursor->hashType || \MonorepoBuilder20210706\Symfony\Component\VarDumper\Cloner\Stub::ARRAY_ASSOC === $cursor->hashType) {
