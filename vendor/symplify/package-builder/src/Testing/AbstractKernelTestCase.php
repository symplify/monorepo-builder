...
namespace MonorepoBuilder20211101\Symplify\PackageBuilder\Testing;
...
use MonorepoBuilder20211101\Symplify\PackageBuilder\Exception\HttpKernel\MissingInterfaceException;
 ...
 */
abstract class AbstractKernelTestCase extends \MonorepoBuilder20211101\PHPUnit\Framework\TestCase
{
    /**
    ...
    protected function getService($type)
    {
        if (self::$container === null) {
            throw new \MonorepoBuilder20211101\Symplify\SymplifyKernel\Exception\ShouldNotHappenException('First, create container with booKernel(KernelClass::class)');
        }
        $service = self::$container->get($type);
        if ($service === null) {
            $errorMessage = \sprintf('Services "%s" was not found', $type);
            throw new \MonorepoBuilder20211101\Symplify\Astral\Exception\ShouldNotHappenException($errorMessage);
        }
        return $service;
    }
    /**
    ...
    protected function bootKernel($kernelClass) : void
    {
        ...
        if (!$kernel instanceof \MonorepoBuilder20211101\Symfony\Component\HttpKernel\KernelInterface) {
    ...
    {
        if (static::$kernel !== null && static::$kernel instanceof \MonorepoBuilder20211101\Symfony\Component\HttpKernel\KernelInterface) {
            ...
            if ($kernel !== null) {
                ...
                if ($container instanceof \MonorepoBuilder20211101\Symfony\Contracts\Service\ResetInterface) {
                    $container->reset();
                }
            }
        }
    ...
    protected function resolveConfigFilePaths($configs) : array
    {
        $configFilePaths = [];
        foreach ($configs as $config) {
            $configFilePaths[] = $config instanceof \MonorepoBuilder20211101\Symplify\SmartFileSystem\SmartFileInfo ? $config->getRealPath() : $config;
        }
    ...
    private function ensureIsConfigAwareKernel($kernel) : void
    {
        ...
        if ($kernel instanceof \MonorepoBuilder20211101\Symplify\PackageBuilder\Contract\HttpKernel\ExtraConfigAwareKernelInterface) {
            return;
        }
        throw new \MonorepoBuilder20211101\Symplify\PackageBuilder\Exception\HttpKernel\MissingInterfaceException(\sprintf('"%s" is missing an "%s" interface', \get_class($kernel), \MonorepoBuilder20211101\Symplify\PackageBuilder\Contract\HttpKernel\ExtraConfigAwareKernelInterface::class));
    }
    private function bootAndReturnKernel(\MonorepoBuilder20211101\Symfony\Component\HttpKernel\KernelInterface $kernel) : \MonorepoBuilder20211101\Symfony\Component\HttpKernel\KernelInterface
    {
        $kernel->boot();
        ...
        if (!$container instanceof \MonorepoBuilder20211101\Symfony\Component\DependencyInjection\ContainerInterface) {
            throw new \MonorepoBuilder20211101\Symplify\SymplifyKernel\Exception\ShouldNotHappenException();
        }
        // has output? keep it silent out of tests
        if ($container->has(\MonorepoBuilder20211101\Symfony\Component\Console\Style\SymfonyStyle::class)) {
            $symfonyStyle = $container->get(\MonorepoBuilder20211101\Symfony\Component\Console\Style\SymfonyStyle::class);
            $symfonyStyle->setVerbosity(\MonorepoBuilder20211101\Symfony\Component\Console\Output\OutputInterface::VERBOSITY_QUIET);
        }
