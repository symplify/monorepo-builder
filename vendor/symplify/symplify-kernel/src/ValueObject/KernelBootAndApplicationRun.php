...
namespace MonorepoBuilder20211130\Symplify\SymplifyKernel\ValueObject;
...
use MonorepoBuilder20211130\Symplify\PackageBuilder\Console\Input\StaticInputDetector;
...
use MonorepoBuilder20211130\Symplify\SymplifyKernel\Exception\BootException;
use Throwable;
/**
 * @api
 */
final class KernelBootAndApplicationRun
{
    ...
    public function run() : void
    {
        try {
            ...
            $symfonyStyleFactory = new \MonorepoBuilder20211130\Symplify\PackageBuilder\Console\Style\SymfonyStyleFactory();
            $symfonyStyle = $symfonyStyleFactory->create();
            $symfonyStyle->error($throwable->getMessage());
            exit(\MonorepoBuilder20211130\Symfony\Component\Console\Command\Command::FAILURE);
        }
    }
    /**
    ...
    private function createKernel()
    {
        ...
        return new $kernelClass($environment, \MonorepoBuilder20211130\Symplify\PackageBuilder\Console\Input\StaticInputDetector::isDebug());
    }
    private function booKernelAndRunApplication() : void
    {
        ...
        $application = $container->get(\MonorepoBuilder20211130\Symfony\Component\Console\Application::class);
    ...
    {
        if (\is_a($kernelClass, \MonorepoBuilder20211130\Symfony\Component\HttpKernel\KernelInterface::class, \true)) {
            return;
        }
        if (\is_a($kernelClass, \MonorepoBuilder20211130\Symplify\SymplifyKernel\Contract\LightKernelInterface::class, \true)) {
            return;
        }
        $errorMessage = \sprintf('Class "%s" must by type of "%s" or "%s"', $kernelClass, \MonorepoBuilder20211130\Symfony\Component\HttpKernel\KernelInterface::class, \MonorepoBuilder20211130\Symplify\SymplifyKernel\Contract\LightKernelInterface::class);
        throw new \MonorepoBuilder20211130\Symplify\SymplifyKernel\Exception\BootException($errorMessage);
    }
}
