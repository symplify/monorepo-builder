...
use MonorepoBuilder20210811\Symplify\SymplifyKernel\Exception\BootException;
use Throwable;
final class KernelBootAndApplicationRun
{
    ...
    private function createKernel() : \MonorepoBuilder20210811\Symfony\Component\HttpKernel\KernelInterface
    {
        ...
        $kernel = new $kernelClass($environment, \MonorepoBuilder20210811\Symplify\PackageBuilder\Console\Input\StaticInputDetector::isDebug());
    ...
    private function booKernelAndRunApplication() : void
    {
        ...
        if ($kernel instanceof \MonorepoBuilder20210811\Symplify\PackageBuilder\Contract\HttpKernel\ExtraConfigAwareKernelInterface && $this->extraConfigs !== []) {
            $kernel->setConfigs($this->extraConfigs);
        }
        $kernel->boot();
        $container = $kernel->getContainer();
        /** @var Application $application */
        $application = $container->get(\MonorepoBuilder20210811\Symfony\Component\Console\Application::class);
        exit($application->run());
    }
    private function setExtraConfigs(\MonorepoBuilder20210811\Symfony\Component\HttpKernel\KernelInterface $kernel, string $kernelClass) : void
    {
        if ($this->extraConfigs === []) {
            return;
        }
        if (\is_a($kernel, \MonorepoBuilder20210811\Symplify\PackageBuilder\Contract\HttpKernel\ExtraConfigAwareKernelInterface::class, \true)) {
            /** @var ExtraConfigAwareKernelInterface $kernel */
            $kernel->setConfigs($this->extraConfigs);
        } else {
            $message = \sprintf('Extra configs are set, but the "%s" kernel class is missing "%s" interface', $kernelClass, \MonorepoBuilder20210811\Symplify\PackageBuilder\Contract\HttpKernel\ExtraConfigAwareKernelInterface::class);
            throw new \MonorepoBuilder20210811\Symplify\SymplifyKernel\Exception\BootException($message);
        }
    }
    /**
     * @param class-string $kernelClass
    ...
    {
        if (!\is_a($kernelClass, \MonorepoBuilder20210811\Symfony\Component\HttpKernel\KernelInterface::class, \true)) {
            $message = \sprintf('Class "%s" must by type of "%s"', $kernelClass, \MonorepoBuilder20210811\Symfony\Component\HttpKernel\KernelInterface::class);
            throw new \MonorepoBuilder20210811\Symplify\SymplifyKernel\Exception\BootException($message);
        }
