...
namespace MonorepoBuilder20220213\Symplify\SmartFileSystem;

use MonorepoBuilder20220213\Nette\Utils\Strings;
use MonorepoBuilder20220213\Symfony\Component\Finder\SplFileInfo;
use MonorepoBuilder20220213\Symplify\EasyTesting\PHPUnit\StaticPHPUnitEnvironment;
use MonorepoBuilder20220213\Symplify\EasyTesting\StaticFixtureSplitter;
use MonorepoBuilder20220213\Symplify\SmartFileSystem\Exception\DirectoryNotFoundException;
use MonorepoBuilder20220213\Symplify\SmartFileSystem\Exception\FileNotFoundException;
/**
 * @see \Symplify\SmartFileSystem\Tests\SmartFileInfo\SmartFileInfoTest
 */
final class SmartFileInfo extends \MonorepoBuilder20220213\Symfony\Component\Finder\SplFileInfo
{
    /**
     * @var string
    ...
    {
        $this->smartFileSystem = new \MonorepoBuilder20220213\Symplify\SmartFileSystem\SmartFileSystem();
        // accepts also dirs
        if (!\file_exists($filePath)) {
            throw new \MonorepoBuilder20220213\Symplify\SmartFileSystem\Exception\FileNotFoundException(\sprintf('File path "%s" was not found while creating "%s" object.', $filePath, self::class));
        }
    ...
    {
        return \MonorepoBuilder20220213\Nette\Utils\Strings::replace($this->getRealPath(), self::LAST_SUFFIX_REGEX, '');
    }
    ...
    public function getRelativeFilePathFromDirectory(string $directory) : string
    {
        if (!\file_exists($directory)) {
            throw new \MonorepoBuilder20220213\Symplify\SmartFileSystem\Exception\DirectoryNotFoundException(\sprintf('Directory "%s" was not found in %s.', $directory, self::class));
        }
    ...
    public function getRelativeFilePathFromCwdInTests() : string
    {
        ...
        if (\MonorepoBuilder20220213\Symplify\EasyTesting\PHPUnit\StaticPHPUnitEnvironment::isPHPUnitRun()) {
            return $this->getRelativeFilePathFromDirectory(\MonorepoBuilder20220213\Symplify\EasyTesting\StaticFixtureSplitter::getTemporaryPath());
        }
