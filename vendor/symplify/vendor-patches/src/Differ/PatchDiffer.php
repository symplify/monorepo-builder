...
namespace MonorepoBuilder20220530\Symplify\VendorPatches\Differ;
...
final class PatchDiffer
{
     ...
     */
    private $differ;
    public function __construct(\MonorepoBuilder20220530\SebastianBergmann\Diff\Differ $differ)
    {
        $this->differ = $differ;
    }
    public function diff(\MonorepoBuilder20220530\Symplify\VendorPatches\ValueObject\OldAndNewFileInfo $oldAndNewFileInfo) : string
    {
        ...
        $clearedDiff = \MonorepoBuilder20220530\Nette\Utils\Strings::replace($diff, self::START_ORIGINAL_REGEX, '--- /dev/null');
        return \MonorepoBuilder20220530\Nette\Utils\Strings::replace($clearedDiff, self::START_NEW_REGEX, '+++ ' . $patchedFileRelativePath);
    }
    private function resolveFileInfoPathRelativeFilePath(\MonorepoBuilder20220530\Symplify\SmartFileSystem\SmartFileInfo $beforeFileInfo) : string
    {
        $match = \MonorepoBuilder20220530\Nette\Utils\Strings::match($beforeFileInfo->getRealPath(), self::LOCAL_PATH_REGEX);
