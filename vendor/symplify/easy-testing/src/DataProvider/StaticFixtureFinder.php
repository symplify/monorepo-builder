...
namespace MonorepoBuilder20210929\Symplify\EasyTesting\DataProvider;

use Iterator;
use MonorepoBuilder20210929\Nette\Utils\Strings;
use MonorepoBuilder20210929\Symfony\Component\Finder\Finder;
...
final class StaticFixtureFinder
{
    ...
    private static function yieldFileInfos(array $fileInfos) : \Iterator
    {
        foreach ($fileInfos as $fileInfo) {
            try {
                $smartFileInfo = new \MonorepoBuilder20210929\Symplify\SmartFileSystem\SmartFileInfo($fileInfo->getRealPath());
                (yield [$smartFileInfo]);
            } catch (\MonorepoBuilder20210929\Symplify\SmartFileSystem\Exception\FileNotFoundException $exception) {
            }
        }
    }
    /**
    ...
    {
        $finder = \MonorepoBuilder20210929\Symfony\Component\Finder\Finder::create()->in($directory)->files()->name($suffix);
    ...
    {
        $iterator = \MonorepoBuilder20210929\Symfony\Component\Finder\Finder::create()->in($directory)->files()->notName($suffix)->getIterator();
        $relativeFilePaths = [];
        foreach ($iterator as $fileInfo) {
            $relativeFilePaths[] = \MonorepoBuilder20210929\Nette\Utils\Strings::substring($fileInfo->getRealPath(), \strlen(\getcwd()) + 1);
        }
        if ($relativeFilePaths === []) {
            return;
        }
        throw new \MonorepoBuilder20210929\Symplify\SymplifyKernel\Exception\ShouldNotHappenException(\sprintf('Files "%s" have invalid suffix, use "%s" suffix instead', \implode('", ', $relativeFilePaths), $suffix));
    }
}
