...
namespace MonorepoBuilder20211101\Symplify\EasyTesting\PHPUnit\Behavior;

use MonorepoBuilder20211101\Symfony\Component\Finder\Finder;
use MonorepoBuilder20211101\Symplify\EasyTesting\ValueObject\ExpectedAndOutputFileInfoPair;
use MonorepoBuilder20211101\Symplify\SmartFileSystem\Finder\FinderSanitizer;
...
trait DirectoryAssertableTrait
{
    ...
    {
        $firstDirectoryFinder = new \MonorepoBuilder20211101\Symfony\Component\Finder\Finder();
        $firstDirectoryFinder->files()->in($directory);
        $finderSanitizer = new \MonorepoBuilder20211101\Symplify\SmartFileSystem\Finder\FinderSanitizer();
    ...
    private function groupFileInfosByRelativeFilePath(array $expectedFileInfos, string $expectedDirectory, array $outputFileInfos, string $outputDirectory) : array
    {
        ...
        foreach ($expectedFileInfos as $expectedFileInfo) {
            ...
            $fileInfosByRelativeFilePath[$relativeFilePath] = new \MonorepoBuilder20211101\Symplify\EasyTesting\ValueObject\ExpectedAndOutputFileInfoPair($expectedFileInfo, $outputFileInfo);
        }
     ...
     */
    private function resolveFileInfoByRelativeFilePath(array $fileInfos, string $directory, string $desiredRelativeFilePath) : ?\MonorepoBuilder20211101\Symplify\SmartFileSystem\SmartFileInfo
    {
