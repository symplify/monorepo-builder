...
final class Random
{
    ...
    public static function generate($length = 10, $charlist = '0-9a-z') : string
    {
        ...
        if ($length < 1) {
            throw new \MonorepoBuilder20210710\Nette\InvalidArgumentException('Length must be greater than zero.');
        } elseif ($chLen < 2) {
            throw new \MonorepoBuilder20210710\Nette\InvalidArgumentException('Character list must contain at least two chars.');
        }
        $res = '';
        for ($i = 0; $i < $length; $i++) {
