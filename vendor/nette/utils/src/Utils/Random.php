...
final class Random
{
    ...
    public static function generate(int $length = 10, string $charlist = '0-9a-z') : string
    {
        ...
        if ($length < 1) {
            throw new \MonorepoBuilder20220108\Nette\InvalidArgumentException('Length must be greater than zero.');
        } elseif ($chLen < 2) {
            throw new \MonorepoBuilder20220108\Nette\InvalidArgumentException('Character list must contain at least two chars.');
        }
        $res = '';
        for ($i = 0; $i < $length; $i++) {
