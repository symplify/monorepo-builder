...
use MonorepoBuilder20220315\Nette\HtmlStringable;
 ...
 */
class Html implements \ArrayAccess, \Countable, \IteratorAggregate, \MonorepoBuilder20220315\Nette\HtmlStringable
{
    ...
    public static function el(?string $name = null, $attrs = null)
    {
        ...
        if (isset($parts[1])) {
            foreach (\MonorepoBuilder20220315\Nette\Utils\Strings::matchAll($parts[1] . ' ', '#([a-z0-9:-]+)(?:=(["\'])?(.*?)(?(2)\\2|\\s))?#i') as $m) {
    ...
    {
        if (!$text instanceof \MonorepoBuilder20220315\Nette\HtmlStringable) {
        ...
        foreach ($attrs as $key => $value) {
            if ($value === null || $value === \false) {
                ...
                if (\strncmp($key, 'data-', 5) === 0) {
                    $value = \MonorepoBuilder20220315\Nette\Utils\Json::encode($value);
                } else {
                    $tmp = null;
