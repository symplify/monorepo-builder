...
final class Callback
{
    ...
    public static function closure($callable, $method = null) : \Closure
    {
        ...
        try {
            ...
            throw new \MonorepoBuilder20210824\Nette\InvalidArgumentException($e->getMessage());
        }
    }
    /**
     * Invokes callback.
    ...
    public static function invokeSafe($function, $args, $onError)
    {
        $prev = \set_error_handler(function ($severity, $message, $file) use($onError, &$prev, $function) : ?bool {
            if ($file === __FILE__) {
                $msg = \ini_get('html_errors') ? \MonorepoBuilder20210824\Nette\Utils\Html::htmlToText($message) : $message;
    ...
    public static function check($callable, $syntax = \false)
    {
        if (!\is_callable($callable, $syntax)) {
            throw new \MonorepoBuilder20210824\Nette\InvalidArgumentException($syntax ? 'Given value is not a callable type.' : \sprintf("Callback '%s' is not callable.", self::toString($callable)));
        }
        return $callable;
    }
    /**
