...
/** @internal */
final class ArrayItemNode extends \MonorepoBuilder20220213\Nette\Neon\Node
{
    /** @var ?Node */
    public $key;
    /** @var Node */
    ...
    public static function itemsToBlockString(array $items) : string
    {
        ...
        foreach ($items as $item) {
            ...
            $res .= ($item->key ? $item->key->toString() . ':' : '-') . ($item->value instanceof \MonorepoBuilder20220213\Nette\Neon\Node\BlockArrayNode && $item->value->items ? "\n" . $v . (\substr($v, -2, 1) === "\n" ? '' : "\n") : ' ' . $v . "\n");
        }
        return $res;
    }
