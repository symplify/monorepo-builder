...
final class Encoder
{
    ...
    public function encode($var, int $flags = 0) : string
    {
        ...
        } elseif ($var instanceof \MonorepoBuilder20211009\Nette\Neon\Entity) {
            if ($var->value === \MonorepoBuilder20211009\Nette\Neon\Neon::CHAIN) {
        ...
        if (\is_array($var)) {
            ...
            if (!\preg_match('~[\\x00-\\x1F]|^[+-.]?\\d|^(true|false|yes|no|on|off|null)$~Di', $var) && \preg_match('~^' . \MonorepoBuilder20211009\Nette\Neon\Decoder::PATTERNS[1] . '$~Dx', $var)) {
                return $var;
            }
            $res = \json_encode($var, \JSON_UNESCAPED_UNICODE | \JSON_UNESCAPED_SLASHES);
            if ($res === \false) {
                throw new \MonorepoBuilder20211009\Nette\Neon\Exception('Invalid UTF-8 sequence: ' . $var);
            }
            if (\strpos($var, "\n") !== \false) {
