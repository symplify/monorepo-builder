...
final class Encoder
{
    ...
    public function valueToNode($val, bool $blockMode = \false) : \MonorepoBuilder20211123\Nette\Neon\Node
    {
        if ($val instanceof \DateTimeInterface) {
            return new \MonorepoBuilder20211123\Nette\Neon\Node\LiteralNode($val);
        } elseif ($val instanceof \MonorepoBuilder20211123\Nette\Neon\Entity && $val->value === \MonorepoBuilder20211123\Nette\Neon\Neon::CHAIN) {
            $node = new \MonorepoBuilder20211123\Nette\Neon\Node\EntityChainNode();
            ...
            return new \MonorepoBuilder20211123\Nette\Neon\Node\StringNode($val);
        } else {
            return new \MonorepoBuilder20211123\Nette\Neon\Node\LiteralNode($val);
        }
    }
    private function arrayToNodes($val, bool $blockMode = \false) : array
    {
        ...
        $hide = \true;
        foreach ($val as $k => $v) {
            $res[] = $item = new \MonorepoBuilder20211123\Nette\Neon\Node\ArrayItemNode();
