...
final class ComposerJsonSymlinker
{
    ...
    public function __construct(\Symplify\MonorepoBuilder\FileSystem\ComposerJsonProvider $composerJsonProvider, \Symplify\MonorepoBuilder\Testing\PathResolver\PackagePathResolver $packagePathResolver, \MonorepoBuilder20220117\Symplify\ComposerJsonManipulator\FileSystem\JsonFileManager $jsonFileManager)
    {
     ...
     * @return mixed[]
     */
    public function decoratePackageComposerJsonWithPackageSymlinks(\MonorepoBuilder20220117\Symplify\SmartFileSystem\SmartFileInfo $packageFileInfo, array $packageNames, \MonorepoBuilder20220117\Symplify\SmartFileSystem\SmartFileInfo $mainComposerJsonFileInfo, bool $symlink) : array
    {
        ...
        foreach ($packageNames as $packageName) {
            ...
            if (\array_key_exists(\MonorepoBuilder20220117\Symplify\ComposerJsonManipulator\ValueObject\ComposerJsonSection::REPOSITORIES, $packageComposerJson)) {
                $packageComposerJson = $this->addRepositoryEntryToPackageComposerJson($packageComposerJson, $repositoriesContentJson);
            } else {
                $packageComposerJson[\MonorepoBuilder20220117\Symplify\ComposerJsonManipulator\ValueObject\ComposerJsonSection::REPOSITORIES][] = $repositoriesContentJson;
            }
        }
    ...
    private function addRepositoryEntryToPackageComposerJson(array $packageComposerJson, array $repositoriesContent) : array
    {
        ...
        foreach ($packageComposerJson[\MonorepoBuilder20220117\Symplify\ComposerJsonManipulator\ValueObject\ComposerJsonSection::REPOSITORIES] as $key => $repository) {
            if ($this->isSamePackageEntry($repository, $repositoriesContent)) {
                ...
                if (isset($repositoriesContent[self::OPTIONS])) {
                    $packageComposerJson[\MonorepoBuilder20220117\Symplify\ComposerJsonManipulator\ValueObject\ComposerJsonSection::REPOSITORIES][$key][self::OPTIONS] = $repositoriesContent[self::OPTIONS];
                } else {
                    unset($packageComposerJson[\MonorepoBuilder20220117\Symplify\ComposerJsonManipulator\ValueObject\ComposerJsonSection::REPOSITORIES][$key][self::OPTIONS]);
                }
                return $packageComposerJson;
            }
        }
        // Add the new entry
        \array_unshift($packageComposerJson[\MonorepoBuilder20220117\Symplify\ComposerJsonManipulator\ValueObject\ComposerJsonSection::REPOSITORIES], $repositoriesContent);
        return $packageComposerJson;
    }
    /**
