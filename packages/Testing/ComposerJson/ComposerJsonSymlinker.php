...
final class ComposerJsonSymlinker
{
    ...
    public function __construct(\Symplify\MonorepoBuilder\FileSystem\ComposerJsonProvider $composerJsonProvider, \Symplify\MonorepoBuilder\Testing\PathResolver\PackagePathResolver $packagePathResolver, \MonorepoBuilder20211111\Symplify\ComposerJsonManipulator\FileSystem\JsonFileManager $jsonFileManager)
    {
     ...
     * @return mixed[]
     */
    public function decoratePackageComposerJsonWithPackageSymlinks(\MonorepoBuilder20211111\Symplify\SmartFileSystem\SmartFileInfo $packageFileInfo, array $packageNames, \MonorepoBuilder20211111\Symplify\SmartFileSystem\SmartFileInfo $mainComposerJsonFileInfo, bool $symlink) : array
    {
        ...
        foreach ($packageNames as $packageName) {
            ...
            if (\array_key_exists(\MonorepoBuilder20211111\Symplify\ComposerJsonManipulator\ValueObject\ComposerJsonSection::REPOSITORIES, $packageComposerJson)) {
                $packageComposerJson = $this->addRepositoryEntryToPackageComposerJson($packageComposerJson, $repositoriesContent);
            } else {
                $packageComposerJson[\MonorepoBuilder20211111\Symplify\ComposerJsonManipulator\ValueObject\ComposerJsonSection::REPOSITORIES][] = $repositoriesContent;
            }
        }
    ...
    private function addRepositoryEntryToPackageComposerJson(array $packageComposerJson, array $repositoriesContent) : array
    {
        ...
        foreach ($packageComposerJson[\MonorepoBuilder20211111\Symplify\ComposerJsonManipulator\ValueObject\ComposerJsonSection::REPOSITORIES] as $key => $repository) {
            if ($this->isSamePackageEntry($repository, $repositoriesContent)) {
                ...
                if (isset($repositoriesContent[self::OPTIONS])) {
                    $packageComposerJson[\MonorepoBuilder20211111\Symplify\ComposerJsonManipulator\ValueObject\ComposerJsonSection::REPOSITORIES][$key][self::OPTIONS] = $repositoriesContent[self::OPTIONS];
                } else {
                    unset($packageComposerJson[\MonorepoBuilder20211111\Symplify\ComposerJsonManipulator\ValueObject\ComposerJsonSection::REPOSITORIES][$key][self::OPTIONS]);
                }
                return $packageComposerJson;
            }
        }
        // Add the new entry
        \array_unshift($packageComposerJson[\MonorepoBuilder20211111\Symplify\ComposerJsonManipulator\ValueObject\ComposerJsonSection::REPOSITORIES], $repositoriesContent);
        return $packageComposerJson;
    }
    /**
