...
final class DependencyUpdater
{
    ...
    public function updateFileInfosWithPackagesAndVersion(array $smartFileInfos, array $packageNames, string $version) : void
    {
        foreach ($smartFileInfos as $smartFileInfo) {
            ...
            $json = $this->processSectionWithPackages($json, $packageNames, $version, \MonorepoBuilder20220522\Symplify\ComposerJsonManipulator\ValueObject\ComposerJsonSection::REQUIRE);
            $json = $this->processSectionWithPackages($json, $packageNames, $version, \MonorepoBuilder20220522\Symplify\ComposerJsonManipulator\ValueObject\ComposerJsonSection::REQUIRE_DEV);
    ...
    public function updateFileInfosWithVendorAndVersion(array $smartFileInfos, string $vendor, string $version) : void
    {
        foreach ($smartFileInfos as $smartFileInfo) {
            $json = $this->jsonFileManager->loadFromFileInfo($smartFileInfo);
            $json = $this->processSection($json, $vendor, $version, \MonorepoBuilder20220522\Symplify\ComposerJsonManipulator\ValueObject\ComposerJsonSection::REQUIRE);
            $json = $this->processSection($json, $vendor, $version, \MonorepoBuilder20220522\Symplify\ComposerJsonManipulator\ValueObject\ComposerJsonSection::REQUIRE_DEV);
            $this->jsonFileManager->printJsonToFileInfo($json, $smartFileInfo);
        }
    }
    /**
