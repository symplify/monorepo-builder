...
use MonorepoBuilder20210705\Symplify\MonorepoBuilder\ValueObject\Package;
...
final class PackageProvider
{
    ...
    public function provide() : array
    {
        ...
        foreach ($this->composerJsonProvider->getPackagesComposerFileInfos() as $packagesComposerFileInfo) {
            ...
            $packages[] = new \MonorepoBuilder20210705\Symplify\MonorepoBuilder\ValueObject\Package($packageName, $hasTests);
        }
        \usort($packages, function (\MonorepoBuilder20210705\Symplify\MonorepoBuilder\ValueObject\Package $firstPackage, \MonorepoBuilder20210705\Symplify\MonorepoBuilder\ValueObject\Package $secondPackage) : int {
            return $firstPackage->getShortName() <=> $secondPackage->getShortName();
        });
        return $packages;
    }
    private function detectNameFromFileInfo(\MonorepoBuilder20210705\Symplify\SmartFileSystem\SmartFileInfo $smartFileInfo) : string
    {
